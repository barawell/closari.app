  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#0D0D0D' }}>Quick Replies</div>
          <div style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>Ketik <code style={{ background: '#F0FDF4', color: '#16A34A', padding: '1px 5px', borderRadius: 3, fontFamily: 'monospace', fontSize: 11 }}>/</code> di Inbox untuk akses cepat.</div>
        </div>
        <button onClick={startNew} style={{ padding: '7px 14px', background: '#0D0D0D', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>+ Tambah</button>
      </div>
      ...
      {loaded && qrs.length === 0 && !showForm && (
        <div style={{ padding: 32, background: '#FAFAFA', border: '1px solid #E5E5E5', borderRadius: 10, textAlign: 'center' }}>
          ...
          <button onClick={startNew} style={{ padding: '7px 14px', background: '#fff', color: '#0D0D0D', border: '1px solid #E5E5E5', borderRadius: 7, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Buat yang pertama</button>
        </div>
      )}
      ...
      {showForm && (
        <div style={{ marginTop: 16, padding: 16, background: '#FAFAFA', border: '1px solid #E5E5E5', borderRadius: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => setShowForm(false)} style={{ padding: '8px 16px', background: '#fff', color: '#6B7280', border: '1px solid #E5E5E5', borderRadius: 7, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Batal</button>
          </div>
        </div>
      )}
    </div>
