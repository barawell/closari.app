...
export default function BroadcastPage() {
  ...
  const [detailFilter, setDetailFilter] = useState<'all' | 'failed' | 'read'>('all')
  ...
  async function openDetail(id: string) {
    setDetailLoading(true); setDetail(null); setDetailFilter('all')
    try {
  ...
  function exportDetailCsv() {
    if (!detail) return
    const rows = [['Nama', 'Nomor', 'Tanggal', 'Status', 'Alasan gagal']]
    for (const r of detail.recipients) rows.push([r.name || '', r.phone || '', r.sent_at ? new Date(r.sent_at).toLocaleString('id-ID') : '', statusLabel(r.status), r.error || ''])
    const csv = rows.map(row => row.map(c => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n')
    const blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `broadcast-${detail.campaign.id}.csv`; a.click(); URL.revokeObjectURL(url)
  }
    ...
    <div className="page-wrap" style={{ maxWidth: 820 }}>
      ...
      {view === 'history' && (
        detailLoading ? (
          ...
          const cFailed = rc.filter((r: any) => r.status === 'failed').length
          const cRead = rc.filter((r: any) => r.status === 'read').length
          const shown = detailFilter === 'all' ? rc : detailFilter === 'failed' ? rc.filter((r: any) => r.status === 'failed') : rc.filter((r: any) => r.status === 'read')
          return (
            <div>
              <button onClick={() => setDetail(null)} style={{ ...btnGhost, fontSize: 12, padding: '6px 12px', marginBottom: 12 }}>← Kembali ke riwayat</button>
              <div style={{ background: '#fff', border: '1px solid #E5E5E5', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '14px 16px', borderBottom: '1px solid #F0F0F0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={exportDetailCsv} style={{ ...btnGhost, fontSize: 12, padding: '6px 12px', flexShrink: 0 }}>⬇ Export CSV</button>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6, padding: '10px 14px', borderBottom: '1px solid #F0F0F0', flexWrap: 'wrap' }}>
                  {[{ k: 'all', l: `Semua (${rc.length})` }, { k: 'failed', l: `Gagal (${cFailed})` }, { k: 'read', l: `Dibaca (${cRead})` }].map(f => (
                    <button key={f.k} onClick={() => setDetailFilter(f.k as any)} style={{ padding: '5px 12px', borderRadius: 999, fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', border: `1px solid ${detailFilter === f.k ? '#16A34A' : '#E5E5E5'}`, background: detailFilter === f.k ? '#F0FDF4' : '#fff', color: detailFilter === f.k ? '#15803D' : '#6B7280' }}>{f.l}</button>
                  ))}
                </div>
                <div>
                  {shown.length === 0 ? (
            ...
            {history.map(c => (
              <div key={c.id} onClick={() => openDetail(c.id)} style={{ background: '#fff', border: '1px solid #E5E5E5', borderRadius: 10, padding: '14px 16px', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', gap: 14, fontSize: 12, alignItems: 'center' }}>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: 12, color: '#16A34A', fontWeight: 500, flexShrink: 0 }}>Lihat detail →</span>
                </div>
              </div>
            ))}
          </div>
