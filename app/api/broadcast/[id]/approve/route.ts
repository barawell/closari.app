// app/api/broadcast/[id]/approve/route.ts
// Approve sebuah campaign 'pending_approval' lalu KIRIM sekarang.
// Hanya role admin/owner yang boleh approve.
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Ambil campaign + pastikan masih pending & milik tenant ini
  const { data: camp } = await supabaseAdmin
    .from('broadcast_campaigns')
    .select('id, tenant_id, wa_number_id, body, engaged_only, status')
  ...
  // Tandai approved (audit) sebelum mulai kirim
  ...
  // Kirim sekarang (helper sudah fix bug ok + handle cooldown)
  const res = await runCampaign({
    campaignId: camp.id,
    tenantId: actor.tenantId,
    phoneNumberId: num.phone_number_id as string,
    accessToken: sec.access_token as string,
    text: camp.body as string,
    engagedOnly: camp.engaged_only !== false,
  })
