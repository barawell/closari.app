...
type DemoMsg = { id: number; from: 'user' | 'ai' | 'agent'; text: string; time: string; kind?: 'text' | 'image' | 'doc' }
...
const DEMO_RESPONSES: Record<string, { intent: string; suggestion: string }> = {
  default: { intent: 'Pertanyaan umum dari customer', suggestion: 'Halo kak! Terima kasih sudah menghubungi kami 🙏 Ada yang bisa kami bantu hari ini?' },
  harga: { intent: 'Menanyakan harga / paket', suggestion: 'Untuk harga, kami punya beberapa paket mulai dari yang basic kak. Boleh tahu dulu kebutuhannya seperti apa? Biar kami rekomendasikan yang paling pas 😊' },
  banned: { intent: 'Masalah nomor kena banned / restrict', suggestion: 'Closari dirancang khusus mencegah nomor kena banned — pakai filter kontak aktif, cooldown otomatis, dan rate aman. Mau kami jelaskan cara kerjanya kak?' },
  fitur: { intent: 'Menanyakan fitur', suggestion: 'Closari punya shared inbox, Aira AI yang balas otomatis & sarankan balasan, broadcast aman, kirim foto/dokumen, sampai follow-up tracking. Fitur mana yang paling kakak butuhkan?' },
  invoice: { intent: 'Minta invoice / dokumen', suggestion: 'Siap kak, invoice-nya kami kirim ya 🧾 Boleh dicek, kalau ada yang kurang jelas langsung kabari aja.' },
  coba: { intent: 'Ingin mencoba / trial', suggestion: 'Gratis untuk dicoba, tanpa kartu kredit kak! Tinggal daftar, connect nomor WA Business, setup-nya cuma ~10 menit. Mau kami bantu?' },
}

function getResponse(text: string) {
  const t = text.toLowerCase()
  if (/(harga|biaya|paket|bayar|tarif)/.test(t)) return DEMO_RESPONSES.harga
  if (/(banned|ban|restrict|blokir|diblokir)/.test(t)) return DEMO_RESPONSES.banned
  if (/(fitur|bisa apa|fungsi)/.test(t)) return DEMO_RESPONSES.fitur
  if (/(invoice|tagihan|dokumen|nota|kwitansi)/.test(t)) return DEMO_RESPONSES.invoice
  if (/(coba|trial|daftar|mulai|gabung)/.test(t)) return DEMO_RESPONSES.coba
...
export default function LiveDemo() {
  ...
  const [messages, setMessages] = useState<Record<number, DemoMsg[]>>({
    0: [
      ...
      { id: 2, from: 'ai', text: 'Halo kak! Closari itu platform WhatsApp Business buat tim CS — bisa shared inbox, AI auto-reply, broadcast aman. Ada yang mau ditanyakan lebih lanjut? 😊', time: '10:23' },
    ],
    1: [{ id: 1, from: 'user', text: 'Nomor WA saya sering kena restrict, gimana solusinya?', time: '10:31' }],
    2: [{ id: 1, from: 'user', text: 'Boleh minta daftar fitur lengkapnya?', time: '10:40' }],
  })
  ...
  useEffect(() => {
    ...
    if (!lastUserMsg) { setCopilot(null); return }
  ...
  function sendUser() {
    ...
    setInput('')
    setSending(true)
    setMessages(prev => ({ ...prev, [activePerson]: [...(prev[activePerson] || []), { id: Date.now(), from: 'user', text: txt, time: now() }] }))
    setTimeout(() => setSending(false), 200)
  }

  function useSuggestion() {
    if (!copilot) return
    setMessages(prev => ({ ...prev, [activePerson]: [...(prev[activePerson] || []), { id: Date.now(), from: 'agent', text: copilot.suggestion, time: now() }] }))
    setCopilot(null)
  }
